...
void Blob::predictNextPosition(void) {
    ...
    if (numPositions == 1) {
        ...
        int deltaX = round((float)sumOfXChanges / 3.0);

        int sumOfYChanges = ((centerPositions[2].y - centerPositions[1].y) * 2) +
            ((centerPositions[1].y - centerPositions[0].y) * 1);

        int deltaY = round((float)sumOfYChanges / 3.0);
        ...
        int deltaX = round((float)sumOfXChanges / 6.0);

        int sumOfYChanges = ((centerPositions[3].y - centerPositions[2].y) * 3) +
            ((centerPositions[2].y - centerPositions[1].y) * 2) +
            ((centerPositions[1].y - centerPositions[0].y) * 1);

        int deltaY = round((float)sumOfYChanges / 6.0);
        ...
        int deltaX = round((float)sumOfXChanges / 10.0);
        ...
        int deltaY = round((float)sumOfYChanges / 10.0);
